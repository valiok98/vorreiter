...
class UserLogin
{
    ...
    public function exists()
    {
        ...
        if ($this->check_by_email) {
            $sql = "SELECT password FROM kunden WHERE email = ?";
        } else {
            $sql = "SELECT password FROM kunden WHERE username = ?";
        }
    ...
    public function login()
    {
        ...
        $sql = '';
        if ($this->check_by_email) {
            $sql = "SELECT id, username, password FROM kunden WHERE email = ?";
        } else {
            $sql = "SELECT id, username, password FROM kunden WHERE username = ?";
        }
                ...
                if ($stmt->num_rows == 1) {
                    ...
                    if ($stmt->fetch()) {
                        if (password_verify($this->password, $this->hashed_password)) {
                            ...
                            header("location:" . URL . "user_content/require_user_pass.php");
                            die;
                        } else {
                    ...
                    $this->username_or_email_error = "No account found with that username.";
                }
            } else {
