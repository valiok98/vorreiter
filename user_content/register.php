...
class CreateUser
{
    ...
    private function check_errors()
    {
        ...
        $sql2 = "SELECT * FROM admin_users WHERE username = ?";
        ...
        if (($stmt1 = $this->mysqli->prepare($sql1)) && ($stmt2 = $this->mysqli->prepare($sql2))) {
            ...
            if ($stmt2->execute()) {
                $stmt2->store_result();
                if ($stmt2->num_rows == 1) {
                    $this->username_error = "This username is already taken by an admin.";
                }
            } else {
                echo "Something went wrong with MySQL.";
            }
        ...
        $sql2 = "SELECT * FROM admin_users WHERE email = ?";
        ...
        if (($stmt1 = $this->mysqli->prepare($sql1)) && ($stmt2 = $this->mysqli->prepare($sql2))) {
            ...
            if ($stmt2->execute()) {
                $stmt2->store_result();
                if ($stmt2->num_rows == 1) {
                    $this->username_error = "This email is already taken by an admin.";
                }
            } else {
                echo "Something went wrong with MySQL.";
            }
